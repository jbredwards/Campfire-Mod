...
/**
 * A config entry implementation that adds functionality for links, and improved translations. Borrowed from Fluidlogged API v3.0.0.
 *
 * @since 2.0.0
 * @author jbred
 *
...
public class ComponentConfigEntry implements GuiConfigEntries.IConfigEntry
{
    ...
    public ComponentConfigEntry(@Nonnull final GuiConfigEntries.IConfigEntry parentIn, @Nonnull final String modId) {
        if(parentIn instanceof GuiConfigEntries.ListEntryBase) {
            ...
            for(@Nonnull final ListIterator<String> it = tooltips.listIterator(); it.hasNext();) {
                ...
                tooltipComponent.getSiblings().forEach(component.appendSibling(tooltipComponent)::appendSibling);
                it.set(tooltipComponent.getFormattedText().replace(oldName, newName));
            }

            if(parentIn.getConfigElement().getType() == ConfigGuiType.STRING) {
                tooltips.remove(tooltips.size() - 1);
                tooltips.add(TextFormatting.AQUA + I18n.format("fml.configgui.tooltip.default", I18n.format("configgui." + modId + ".default." + parentIn.getConfigElement().getDefault())));
            }
        }
