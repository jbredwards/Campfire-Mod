...
public final class ModelCampfireLogs implements IModel
{
    ...
    @Nonnull
    @Override
    public Collection<ResourceLocation> getDependencies() { return Collections.singletonList(parent); }
    ...
    public IBakedModel bake(@Nonnull IModelState state, @Nonnull VertexFormat format, @Nonnull Function<ResourceLocation, TextureAtlasSprite> bakedTextureGetter) {
        return new BakedModel(ModelLoaderRegistry.getModelOrLogError(parent, "Couldn't load Campfire log model dependency: " + parent).bake(state, format, bakedTextureGetter), null);
    }
    ...
    {
        @Nullable
        final CampfireWoodType<?> forcedType;
        BakedModel(@Nonnull IBakedModel originalModel, @Nullable CampfireWoodType<?> forcedTypeIn) {
            super(originalModel);
        ...
        public List<BakedQuad> getQuads(@Nullable IBlockState state, @Nullable EnumFacing side, long rand) {
            final CampfireWoodType<?> type = forcedType == null && state instanceof IExtendedBlockState ? ((IExtendedBlockState)state).getValue(CampfireWoodTypeProperty.INSTANCE) : forcedType;
            final List<BakedQuad> bakedQuads = super.getQuads(state, side, rand);
            if(type == null) return bakedQuads;

            //get side texture
            final IBakedModel itemModel = type.getModel();
        ...
        public ItemOverrideList getOverrides() {
            return new ItemOverrideList(Collections.emptyList()) {
                ...
                public IBakedModel handleItemState(@Nonnull IBakedModel originalModelIn, @Nonnull ItemStack stack, @Nullable World world, @Nullable EntityLivingBase entity) {
                    return Optional.ofNullable(ICampfireWoodType.get(stack)).flatMap(ICampfireWoodType::getOptional).<IBakedModel>map(wt -> new BakedModel(originalModel, wt)).orElse(originalModelIn);
                }
            };
        }
    }
