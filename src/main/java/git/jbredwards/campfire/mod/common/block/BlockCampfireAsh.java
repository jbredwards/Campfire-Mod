...
public class BlockCampfireAsh extends BlockFalling implements IFluidloggable
{
    ...
    public BlockCampfireAsh(@Nonnull Material materialIn, @Nonnull MapColor mapColorIn) {
        super(materialIn);
        blockMapColor = mapColorIn;
        setDefaultState(getDefaultState().withProperty(LAYERS, 1));
        setSoundType(SoundType.SAND).setCreativeTab(CampfireCreativeTab.INSTANCE)
    ...
    public void randomDisplayTick(@Nonnull IBlockState stateIn, @Nonnull World worldIn, @Nonnull BlockPos pos, @Nonnull Random rand) {
        ...
        if(CampfireConfigHandler.Campfire.campfireAshEmitsParticles && rand.nextFloat() < 0.25) {
            ...
            for(EnumFacing side : EnumFacing.values()) {
                if(stateIn.shouldSideBeRendered(worldIn, pos, side) && (side.getAxis().isVertical() || rand.nextFloat() < yOffset)) {
                    ...
                    worldIn.spawnParticle(getDustParticle(), x, y, z, 0, 0, 0);
                }
            }
        }
    }
    ...
    @Nonnull
    protected EnumParticleTypes getDustParticle() { return EnumParticleTypes.SMOKE_NORMAL; }
}
