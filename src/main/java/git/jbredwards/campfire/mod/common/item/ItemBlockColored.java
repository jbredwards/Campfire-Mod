...
public class ItemBlockColored extends ItemBlock
{
    ...
    //tell the user this item is dyeable! Ignored if the tooltip translation is empty.
    ...
    public void addInformation(@Nonnull final ItemStack stack, @Nullable final World worldIn, @Nonnull final List<String> tooltip, @Nonnull final ITooltipFlag flagIn) {
        if(stack.getMetadata() == 0 && getColor(stack) == -1) {
            @Nonnull final String dyeableTooltip = I18n.format("tooltip.campfire.dyeable");
            if(!dyeableTooltip.isEmpty()) tooltip.add(dyeableTooltip);
        }
    }*/
